...
export const createBulkSensorData = async (req: Request, res: Response) => {
  try {
    console.log('\n📥 ==> IOT REQUEST: Bulk sensor data received');
    console.log('🕐 Timestamp:', new Date().toISOString());
    console.log('📊 Request body:', JSON.stringify(req.body, null, 2));
    
    const { data } = req.body;
    if (!Array.isArray(data) || data.length === 0) {
      console.log('❌ Invalid request: data array missing or empty');
      const response: ApiResponse<null> = {
        success: false,
    ...
    console.log(`📈 Processing ${data.length} sensor readings...`);
    ...
    if (sensors.length !== sensorIds.length) {
      console.log('❌ Some sensors not found or inactive');
      const response: ApiResponse<null> = {
        success: false,
    ...
    console.log(`✅ SUCCESS: Created ${sensorDataRecords.length} sensor records in database`);
    console.log('📤 <== Sending success response to IoT client\n');
    ...
    console.error('\n❌ BACKEND ERROR in createBulkSensorData:', error);
    const response: ApiResponse<null> = {
      success: false,
